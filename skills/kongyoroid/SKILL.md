...
フロントエンド (jpreprocess + NAIST-JDic) は**読めなかった部分を黙って落とす**か、**もっともらしい別の読みを返す**ことがある。どちらも警告なしで `ok: true` になる。実測例:

| 入力 | 実際の読み | 起きたこと |
| --- | --- | --- |
| `二文目。` | `ニア'ヤメ。` | 誤読。警告なし |
| `テストが3件失敗しました。` | `テ'_ストガ/サン'ゲン/…` | 助数詞が サンケン ではなく サンゲン |
| `完了しました🎉` | `カンリョー/シマ'_シタ。` | 絵文字が消える。警告なし |
| `简体` | `タイ。` | 簡体字専用の字が消える。警告なし |
| `user@example.com` | `ユーエスイーアール、アットマ'ーク、…` | 1 文字ずつ読む。警告なし |

だから**必ず `reading` か `plan` の `kana` を原稿と突き合わせる**。固有名詞・数詞と助数詞・同音異義語・英単語・記号を優先して見る。読み違いを見つけたら `dictionary` か `kana` で直す (→ [speech.md](references/speech.md))。
...
```sh
npm install --save-exact @kongyo2/kongyoroid@2.0.0
