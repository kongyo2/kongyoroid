...
文の切れ目は `。！？` と改行。次の文を先読みして合成するので途切れない。文間ポーズは次の文の先頭に付き、`postPause` は最後に 1 回だけなので、**一括合成と同じ尺**になる。第 2 引数は `text` を除いた `SpeakInput`、第 3 引数で `blockFrames` / `flushMs` / `maxSentenceChars` を調整できる。
...
`directory` を渡すと `directory/kongyoroid-cache/` の中だけを使い、外のファイルには触らない。キーはエンジン・パッケージ版・エンジン版・フロントエンド版・辞書ダイジェスト・正規化リクエストから作るので、更新後に古い音声が返ることはない。同じリクエストが同時に来ても合成は 1 回だけ (in-flight dedupe)。実測でヒット時は 760 ms → 4.6 ms。
